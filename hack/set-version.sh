...
sed -i.bak -e "s@newTag:[[:space:]]*v${CURRENT_VERSION}@newTag: v${NEW_VERSION}@g" ${BASE_DIR}/config/openai-gateway/kustomization.yaml
sed -i.bak -e "s@RELEASE_VERSION[[:space:]]*=[[:space:]]*\"${CURRENT_VERSION}\"@RELEASE_VERSION = \"${NEW_VERSION}\"@g" ${BASE_DIR}/version.go

make build-installer IMG=norseto/openai-gateway:v$(hack/get-version.sh)
